...
exports.secureGetContentById = async (req, res) => {
  ...
  try {
    ...
    if(tiket_value.stat[1] === "byid" && tiket_value.stat[0] === key2) {
      try {
        const ContentbyId = await homeContent.findById(tiket_value.id); // Mengambil 1 data berita
        res.status(200).json({
...
exports.secureGetAllContent = async (req, res) => {
  ...
  try {
    ...
    if(tiket_value.stat[1] === "all" && tiket_value.stat[0] === key2) {
      try {
        const AllContent = await homeContent.find(); // Mengambil 1 data berita
        res.status(200).json({
          message: 'Berhasil mendapatkan content',
          Content: AllContent
        });
...
exports.Content = async (req, res) => {
  try {
    const ContentBaru = new homeContent(req.body); // Data menu dikirimkan melalui body request
...
exports.getSemuaContent = async (req, res) => {
  try {
    const semuaContent = await homeContent.find(); // Mengambil semua data menu
    res.status(200).json({
...
exports.getContentById = async (req, res) => {
  try {
    const content = await homeContent.findById(req.params.id); // Mengambil menu berdasarkan ID
    if (!content) {
...
exports.updateContent = async (req, res) => {
  try {
    const contentDiupdate = await homeContent.findByIdAndUpdate(req.params.id, req.body, {
...
exports.deleteContent = async (req, res) => {
  try {
    const contentDihapus = await homeContent.findByIdAndDelete(req.params.id);
    if (!contentDihapus) {
      return res.status(404).json({
        message: 'Content tidak ditemukan'
      });
